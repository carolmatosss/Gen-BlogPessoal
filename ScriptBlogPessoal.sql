...
-- -----------------------------------------------------
-- Table `db_blog_pessoal`.`tb_usuário`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `db_blog_pessoal`.`tb_usuário` (
  `id` BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `db_blog_pessoal`.`tb_postagens` (
  ...
  INDEX `fk_tb_postagens_tb_usuário1_idx` (`tb_usuário_id` ASC) VISIBLE,
  CONSTRAINT `fk_tb_postagens_tb_temas`
    FOREIGN KEY (`tb_temas_id`)
    REFERENCES `db_blog_pessoal`.`tb_temas` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_postagens_tb_usuário1`
    FOREIGN KEY (`tb_usuário_id`)
    REFERENCES `db_blog_pessoal`.`tb_usuário` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
